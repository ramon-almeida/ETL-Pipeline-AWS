...
for index, row in df.iterrows():
    insert = f"""
    INSERT INTO orders (order_id, order_time, store_name, total_price, payment_type) values ({int(row['order_id'])}, STR_TO_DATE('{row['order_time']}', '%d/%m/%Y %H:%i'), '{row['store_name']}', {float(row['total_price'])}, '{row['payment_type']}')
    """
    mysql_execute_commit(conn, insert)
